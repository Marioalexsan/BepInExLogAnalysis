...
public partial class LogAnalyzer
{
    ...
    public async Task<LogReport> ProcessLogAsync(Stream input, CancellationToken ct = default)
    {
        ...
        SplitLines(reader, report, (line, lineIndex, reachedEnd) =>
        {
    ...
    private void SplitLines(StreamReader reader, LogReport report, Action<ReadOnlySpan<char>, int, bool> callback)
    {
        ...
        while (true)
        {
            ...
            if (bufferCount == buffer.Length)
            {
                ...
                if (!dropLine)
                    report.ProcessingErrors.Add($"Line {lineIndex + 1} is over the length limit and will be ignored.");
